...
There is also X11 support, and will be automatically handled by CMake if you are compiling within a X11 environment.
In this scenario, GLFW will be used to handle window management and input events, with direct X11 manipulation of the
window to handle input events.
...
macOS is also not supported, but it should be possible to run it on macOS with some modifications, like switching
from OpenGLES to regular OpenGL.
